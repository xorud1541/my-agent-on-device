...
### 벤치마크 결과 (2026-06-11, Intel Arc iGPU Vulkan0, llama.cpp b9334)

| 모델 | pp512 t/s | tg128 t/s | 한국어 툴콜 (5 시나리오) | 호출당 레이턴시 |
|------|----------:|----------:|:---:|:---:|
| **Qwen3.5-2B Q4_K_M (선정)** | 577 | 19.3 | 5/5 | 3.5~6.5s |
| supergemma4 E4B Q4_K_S | 293 | 10.5 | 5/5 | 10~18s |
| gemma-4-E4B Q4_K_S | 287 | 10.1 | (미실시, supergemma4 와 동일 베이스) | - |
| Qwen3.5-4B Q4_K_M | 107 | 7.8 | (속도 탈락) | - |
| Llama-3.1-8B Q4_K_M | 182 | 7.1 | (속도 탈락) | - |

**선정: Qwen3.5-2B-Q4_K_M.** 툴콜 정확도 만점 + 턴당(2~3회 호출) 10~20초로 목표 여유 충족.
품질이 더 필요한 경우 설정에서 supergemma4 로 교체 가능 (턴당 ~35-50초, 목표 상한 근접).
재현: `bench/toolcall_test.mjs`, llama-bench 명령은 git 이력 참고.

### 정정 + 후속 검증 (2026-06-11 저녁 재측정)

위 표의 절대치는 측정 당시 iGPU 상태 문제로 **약 4배 낮게** 측정된 것 (상대 비율은 대체로 유지).
재측정: Qwen3.5-2B Q4 **pp 2509 / tg 72.8**, gemma-4-E2B 2431/63.4, supergemma4-E4B 1223/36.8,
Qwen3.5-4B 466/8.9 (dense 라 대역폭 병목 — 진짜 느림).

중간 체급 후보 전수 검증 결과 (`gt_eval.mjs`, GT_BASE 환경변수로 후보 서버 평가 가능):
- **gemma-4 계열(E2B/E4B)은 에이전트 부적합** — 빠르지만 GT 60건에서 4~11/60.
  도구 호출 대신 "어디에 저장할까요?" 되묻기로 일관, 되묻기 금지 규칙을 넣어도 교정 불가 (튜닝 특성).
- **Vulkan(Intel iGPU)은 Q4_K 계열만 빠름** — Qwen2B Q8_0 tg 16.4, Q6_K 13.5 로 Q4_K_M(72.8)의 1/4~1/5.
  고정밀 양자화로 품질을 올리는 경로는 이 백엔드에선 불성립.
- 결론: Qwen3.5-2B-Q4_K_M 유지. 능력 환각은 시스템 프롬프트 규칙 11(긍정문 나열)로 완화
  (부정문 규칙은 모델이 "X 못 한다" 토큰을 베껴 역효과 — agent.rs 테스트로 고정).
