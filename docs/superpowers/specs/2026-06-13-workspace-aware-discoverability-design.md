...
## 6.5 개정 (2026-06-14) — 칩 대신 에이전트 말풍선

사용자 피드백: "한 줄 요약 + 버튼형 제안이 별로다. 워크스페이스 간단 요약과 기능 제안을
**에이전트 말풍선**으로 보여달라." → 빈 화면 표현을 다음과 같이 바꾼다(백엔드/타입/useAgent/테스트 불변):

- **합성 어시스턴트 말풍선(결정적, 모델 호출 없음)**: `messages.length === 0`일 때, `summary` 데이터로
  마크다운 텍스트를 조립해 `msg-assistant` + `prose`(ReactMarkdown) 구조로 렌더 — 진짜 어시스턴트
  메시지처럼 보이는 첫 인사. 대화 상태(`messages`)에는 넣지 않는다(세션/모델 비오염, 첫 전송 시 사라짐).
- **제안은 순수 텍스트(클릭 없음)**: 버튼/칩 제거. 말풍선 안에 `예를 들어 이렇게 말해보세요:` +
  `summary.suggestions`를 따옴표 친 불릿으로 보여준다(사용자가 직접 타이핑). → **Composer prefill(제안 클릭)
  메커니즘 제거** — Task 5와 Task 6의 prefill 배선을 되돌린다.
- **상태별 말풍선**:
  - ① 폴더+파일: `📁 **{folder}** 폴더에 {이미지 N장, PDF N개, zip N개}가 있어요.` + `예를 들어 이렇게 말해보세요:` + 따옴표 불릿(`summary.suggestions`).
  - ①' 폴더+다룰 파일 없음: `📁 **{folder}** 폴더에는 아직 다룰 수 있는 파일(이미지·PDF·zip)이 없어요.` + 폴더 변경/캡처 안내.
  - ② 홈/첫 실행/요약 로딩 전(null): `안녕하세요! …도와드려요. 먼저 작업할 폴더를 골라주세요.`
- **폴더 선택**: ②/①'에서는 `📁 작업할 폴더 선택` 버튼 1개를 말풍선 아래 유지(이건 *제안*이 아니라 필수
  컨트롤 — 폴더 지정은 타이핑으로 대체 불가). `pick_folder` + `set_config` 재사용.
- 말풍선 텍스트 조립은 프론트(App)에서 수행(이미 백엔드에서 검증된 결정적 데이터의 단순 포매팅).
