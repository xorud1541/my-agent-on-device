...
### Addendum 3 (2026-06-13): OS 네이티브 화면 오버레이 영역 캡처로 전환 ★현행

**Addendum 2의 "앱 내 모달" 방식을 폐기하고 OS 네이티브 인터랙티브 영역 캡처로 바꾼다.**

동기:
- 속도: 앱 내 모달 방식은 매번 화면 전체를 캡처 → 인코딩 → (다운스케일 프리뷰라도) base64로
  IPC 전송 → 모달 표시까지 지연이 있다. 📷 누르고 선택 화면이 뜨기까지 느리다.
- UX: 캡처본/선택 화면이 "앱 창 안"에 뜨는 게 어색하다. 사용자는 **OS 스크린샷처럼** 동작하길 원한다:
  📷 클릭 → **전체 화면에 살짝 음영** → 사용자가 **실제 화면 위에서 영역 드래그** → 그 영역만 캡처본.

목표 UX (사용자 요구):
1. 📷 클릭 시 전체 화면에 살짝 음영(dim)이 깔린다.
2. 사용자가 그 위에서 사각형으로 영역을 지정한다.
3. 지정한 영역만 캡처본이 된다.
4. 캡처본(썸네일)은 입력창 위에 첨부된다. (기존 유지)

구현 방식 — **OS 네이티브 인터랙티브 캡처 도구**(webview 오버레이 창을 만들지 않으므로 Addendum 2의
WebKit 크래시와 무관, 별도 프로세스라 안전):
- **macOS**: `screencapture -i [-x] <cache>/captures/capture_<ts>.png`
  - `-i` 인터랙티브 = OS가 화면에 음영/크로스헤어를 띄우고 드래그 선택을 받는다(요구한 UX 그대로).
    멀티모니터에서도 사용자가 원하는 화면/영역을 직접 고른다(→ "현재 모니터" 판정 불필요).
  - `-x` 는 셔터음 무음(선택).
  - 사용자가 영역을 놓으면 **그 영역만** PNG로 저장된다. Esc/취소 시 **파일이 생성되지 않음** → 첨부 없음.
  - 앱 자신이 찍히지 않도록 캡처 직전 앱 창 `hide()`, 완료 후 `show()`.
- **Windows**: 네이티브 인터랙티브 영역 캡처 = **`ms-screenclip:` (Windows 10/11 기본 화면 캡처
  오버레이) + 클립보드 이미지 읽기**. 이것도 OS가 화면에 음영을 깔고 드래그 선택을 받는 동일한 UX다.
  Windows에는 "영역만 파일로 바로 저장"하는 표준 CLI가 없어 결과가 **클립보드**로 오므로, 클립보드에서
  이미지를 읽어 캐시에 PNG로 저장한다. 확정 구현 흐름:
  1. 앱 `hide()`.
  2. 캡처 시작 전 클립보드 상태를 스냅샷(직전 이미지가 있으면 그 식별값/없음을 기록) — 새 캡처 판별용.
  3. 오버레이 실행: `cmd /c start ms-screenclip:` (또는 동등 실행). 사용자가 화면 음영 위에서 영역 드래그
     → 스니핑 결과가 클립보드로 복사된다.
  4. 클립보드를 폴링(`arboard` 크레이트 `Clipboard::get_image()`)하여 **2의 스냅샷과 다른 새 이미지**가
     나타나면 그 RGBA 버퍼를 PNG로 저장. 최대 대기 60초.
  5. 앱 `show()`/`set_focus()`. 타임아웃/새 이미지 없음(취소) → `Ok(None)`.
  - `arboard` 는 크로스플랫폼 클립보드 크레이트로 Windows 이미지 읽기를 지원한다(`[target.'cfg(windows)'.
    dependencies]` 에 추가). macOS 경로는 클립보드를 쓰지 않으므로 영향 없음.
  - 폴링 간격/타임아웃, 사용자가 스니핑을 취소(Esc)했을 때 새 클립보드 이미지가 없으면 None 으로 귀결되는
    경로를 명확히 구현한다.
  - 대안(보류): `SnippingTool.exe /clip` 는 구버전/동작 편차가 있어 1순위는 `ms-screenclip:`.

속도 이점: OS가 **선택 영역만** 디스크에 직접 저장 → 전체 화면 캡처/인코딩/대용량 base64 IPC가 전부
사라진다. 백엔드는 저장된 작은 PNG로 썸네일만 만들면 된다 → 체감 즉시.

제거되는 것 (Addendum 2 산물):
- 앱 내 모달 컴포넌트 `RegionOverlay`(메인 창 안 전체화면 모달).
- 전체 화면 캡처 + 프리뷰 생성(`capture_to_cache` 의 full PNG + 다운스케일 JPEG), `crop_capture`,
  정규화 좌표 환산. 더 이상 필요 없다.

백엔드 (commands.rs):
- `capture_region(app) -> Result<Option<CaptureResult>, String>` 단일 커맨드.
  - 앱 `hide()` → `spawn_blocking` 으로 OS 인터랙티브 캡처 실행(블로킹) → 앱 `show()`/`set_focus()`.
  - 결과 파일이 존재하면 320px 썸네일 생성 후 `CaptureResult{path, thumb_data_url, width, height}`,
    파일이 없으면(취소) `Ok(None)`.
  - OS 분기는 `#[cfg(target_os = ...)]`. (네이티브 도구는 OS별이 정석 — 단일-코드패스 원칙은 webview
    오버레이를 쓸 때만 유효했고, 이제 적용 대상이 아니다.)
- 캡처 원본은 앱 캐시(`app_cache_dir()/captures/`), 도구 산출물만 워크스페이스(기존 가드 유지).

프론트:
- `Composer` 📷 → `invoke("capture_region")` → 결과 있으면 첨부 칩 추가, `null`이면 무시(취소).
- `pending`/모달/프리뷰 관련 상태와 `RegionOverlay` import 제거. 첨부 칩/말풍선 썸네일은 그대로.
- 캡처 진행 중 버튼 비활성/스피너 정도만 유지.

크로스플랫폼 (필수 — 둘 다 구현):
- **macOS**: `screencapture -i`(파일 직접 저장). 현재 사용 환경이라 우선 구현·검증.
- **Windows**: `ms-screenclip:` + `arboard` 클립보드 읽기(위 흐름). 배포 타깃이 Windows(NSIS)이므로
  **반드시 함께 구현**한다. 두 경로는 `#[cfg(target_os = "macos")]` / `#[cfg(target_os = "windows")]`
  로 분기하고, 그 외 OS(Linux 등)는 명확한 미지원 에러를 반환한다.
- 공통 후처리(파일 → 320px 썸네일 → `CaptureResult`)와 프론트 첨부 흐름은 OS 무관하게 단일 코드.

완료 기준:
1. `cargo build`(+ Windows 타깃) / `npx tsc --noEmit` / `pnpm build` 성공.
2. **macOS·Windows 양쪽에서** 📷 → 전체 화면 음영 + 드래그(OS 네이티브), 지정 영역만 첨부.
3. 이전(앱 내 모달)보다 체감상 빠르다(전체 캡처/IPC 제거).
4. 취소(Esc) 시 첨부 없이 정상 복귀, 크래시 없음 — 양 OS 동일.
5. 크롭 결과 경로가 기존 첨부/vision/도구 파이프라인에 그대로 연결.
6. Windows 클립보드 폴링의 타임아웃/취소 경로가 무한 대기 없이 안전하게 종결.
