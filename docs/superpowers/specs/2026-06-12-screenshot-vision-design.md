...
- 캡처 원본은 **워크스페이스 `captures/`** 에 저장한다(2026-06-13 사용자 요청 — 캐시 임시저장에서
  변경, 기존 `screen_capture` 도구와 동일 관례). 도구 산출물도 워크스페이스(기존 가드).
  입력이 이미 워크스페이스 안이므로 배경제거/변환의 기본 산출물도 자연히 워크스페이스로 떨어진다.
