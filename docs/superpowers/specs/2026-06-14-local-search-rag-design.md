...
const RAG_MIN_COSINE: f32 = 0.45;  // 빌드/모델 바뀌면 재보정
...
- `RAG_MIN_COSINE=0.45`는 대부분의 정답을 통과시키나 **하한에 가깝다**(한 정답이 0.455). 모호 질의의 오답(0.473)도 통과 → recall 우선·precision 손해. 실데이터로 추가 보정 여지.
